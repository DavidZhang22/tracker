...
`sources.json`, independent of the tracker's current relevance rules. A positive
...
`dataset.jsonl` contains frozen features, public URL/title metadata, labels and
split identifiers. `dataset-card.json` records capture hashes and the labeling
rules. `holdout-dataset.jsonl` is kept separate so it cannot accidentally enter
...
frozen. `initial-test-report.json` preserves that first result. Consequently,
`test-report.json` is now a **development regression result**, not an unbiased
...
itself needs only frozen `dataset.jsonl`; exported predictions are checked against
