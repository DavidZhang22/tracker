...
from ml.semantic_decisions.data import extract_page
...
def sha(path):
    return hashlib.sha256(artifacts.read_bytes(path)).hexdigest()


def frozen(path, value):
    if artifacts.exists(path):
        if artifacts.read_json(path) != value:
            raise ValueError(
...
def load_baseline(directory):
    ...
    if not artifacts.exists(path):
        artifacts.write_bytes(
            path, artifacts.read_bytes(ROOT / "app/tracker/link-cascade-model.json")
        )
    model = CascadeModel(json.loads(artifacts.read_text(path, encoding="utf8")))
...
def sources_and_roles():
    prior = json.loads(
        artifacts.read_text(
            ROOT / "ml/experiments/breadth-generalization/protocol.json"
        )
    )
    breadth_roles = {
        k: v
    ...
    for name in HISTORY:
        for line in artifacts.read_text(
            ROOT / "ml/datasets" / name, encoding="utf8"
        ).splitlines():
            row = json.loads(line)
    ...
    for name in HISTORICAL_MANIFESTS:
        ...
        paths.append(path)
        for s in json.loads(artifacts.read_text(path, encoding="utf8")):
...
def prepare_page(source, cache, baseline):
    ...
    identity = digest_json(
        dict(
            source=source,
            capture=digest,
            extractor=sha(Path(__file__).with_name("data.py")),
    ...
    if artifacts.exists(path):
        page = json.loads(artifacts.read_text(path, encoding="utf8"))
...
def prepare(directory, cache):
    ...
    protocol = dict(
        ...
        extractor_sha256=sha(Path(__file__).with_name("data.py")),
...
def fit(directory, cache):
    ...
    for name, (encoder, kind) in SPECS.items():
        ...
        report["timings"][name] = dict(
            ...
            artifact_bytes=len(artifacts.read_bytes(directory / (name + ".json"))),
...
def evaluate(directory, cache, manifest, output):
    selection = json.loads(artifacts.read_text(directory / "selection.json"))
    ...
    baseline = load_baseline(directory)
    protocol = json.loads(artifacts.read_text(directory / "protocol.json"))
    for expected, actual in (
        (protocol["baseline_sha256"], sha(directory / "baseline.json")),
        (
            protocol["extractor_sha256"],
            sha(Path(__file__).with_name("data.py")),
        ),
        (
    ...
    training_summary = json.loads(artifacts.read_text(directory / "data-summary.json"))
    if manifest:
        sources = [
            dict(s, split="fresh", cohort="fresh")
            for s in json.loads(artifacts.read_text(manifest))
            if s.get("status") == "captured" and s.get("expected_urls")
        ]
        ...
        for path in (ROOT / "ml/datasets").glob("*.jsonl"):
            if path.name.startswith("semantic-decision-holdout"):
                continue
            for line in artifacts.read_text(path, encoding="utf8").splitlines():
                r = json.loads(line)
        ...
        pages = [
            json.loads(artifacts.read_text(cache / (p["id"] + ".json")))
    ...
    for finalist in {
        ...
        payload = json.loads(artifacts.read_text(directory / (name + ".json")))
