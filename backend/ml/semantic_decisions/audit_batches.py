...
def main():
    ...
    name = selection["name"]
    manifest = json.loads(
        artifacts.read_text(ROOT / "ml/datasets/semantic-decision-holdout-sources.json")
    )
    pages = [
        json.loads(artifacts.read_text(cache / (s["id"] + ".json")))
        for s in manifest
    ...
    runtime = json.loads(artifacts.read_text(args.runtime))
