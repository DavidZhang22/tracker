...
def read_json(path):
    return json.loads(artifacts.read_text(Path(path), encoding="utf8"))
...
def model_files(directory, case):
    ...
    return [
        dict(
            path=str(path), bytes=len(artifacts.read_bytes(path)), sha256=file_sha(path)
        )
        for path in paths
    ]
...
def worker(request, output):
    ...
    forbidden = [
        name
        for name in ("sklearn", "torch", "ml.semantic_decisions.experiment")
...
def process_memory(pid):
    ...
    if sys.platform.startswith("linux"):
        values = {}
        for line in artifacts.read_text(Path(f"/proc/{pid}/status")).splitlines():
    ...
    with tempfile.TemporaryDirectory(prefix="semantic-runtime-") as temporary:
        ...
        if child.returncode:
            raise RuntimeError(
                ...
                + artifacts.read_text(
                    folder / "worker.log", encoding="utf8", errors="replace"
                )
            )
        if not samples:
...
def main():
    ...
    selection = read_json(selection_path) if artifacts.exists(selection_path) else None
    finalists = {}
    if selection:
        for filename, key in (
