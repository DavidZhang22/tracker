...
`datasets/record-context-dataset.jsonl` has 31,113 candidate regions: 22,666 training,
...
Training writes `ml/experiments/records/`; promotion copies only its validated
...
`experiments/records/context-benchmark.json` and `experiments/records/context-live-check.json`.
