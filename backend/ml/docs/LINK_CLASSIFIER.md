...
`datasets/v3-dataset.jsonl` contains 11,837 rows: 8,429 training, 2,607 validation and 801
...
```sh
uv sync --group ml
uv run --group ml python ml/train_context.py --expanded --dataset ml/datasets/v3-dataset.jsonl --output-dir ml/experiments/tables --fallback-model ml/experiments/context/link-context-model.json
uv run pytest -q
...
```sh
uv run python ml/build_v2_dataset.py --output ml/datasets/v3-base-dataset.jsonl
uv run python ml/build_table_dataset.py --base ml/datasets/v3-base-dataset.jsonl
```
...
Selection first requires 95% aggregate precision, then maximizes site macro F1. Candidate order breaks ties toward smaller models. Export predictions match scikit-learn within **1.16e-8**. Scores are uncalibrated, not reliable probabilities of correctness. See `experiments/context/training-report.json` for all thresholds and per-site measurements.

## Dataset and provenance

`datasets/v2-dataset.jsonl` contains **9,999 anchor-context rows**, not independent pages:
...
Reports count unique source/URL pairs and list predictions outside the labeled universe. `reports/context-train-report.json` evaluates uncapped snapshots while fitting uses capped rows. Baseline source is retained in `baselines/v1/parser.py`. All v1 test sites are development data now. The model cannot expose content absent from fetched HTML/API responses.
...
Training is offline from the frozen JSONL, uses one BLAS thread and seed 81, and writes a **candidate** under `ml/experiments/context`. It never replaces the bundled artifact automatically. Review per-site regressions and evaluate new untouched sites before promotion. Reusing the current test set makes it development data. All candidates trained locally in about 21 seconds. Peak Python allocations during model loading were about 0.7 MiB; VM measurements are in `reports/context-benchmark.json`.
